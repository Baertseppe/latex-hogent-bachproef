...
Andere mogelijke databronnen voor Europese competities werden overwogen, maar bleken minder volledig of minder consistent voor de BNXT-context. Daarom is bewust gekozen om de officiële BNXT-website en RealGM als primaire bronnen te gebruiken.
...
\begin{table}[H]
    \centering
    \caption{Overzicht van gebruikte statistische afkortingen.}
    \label{tab:stat-afkortingen}
    \begin{tabular}{p{0.4\textwidth} r}
        \toprule
        \textbf{Afkorting} & \textbf{Betekenis} \\
        \midrule
        PTS   & Points (totaal aantal gescoorde punten) \\
        REB   & Rebounds (totaal aantal rebounds) \\
        ...
        MIN   & Minutes played (gespeelde minuten) \\
        EFF   & Efficiëntiescore \\
        \bottomrule
    \end{tabular}
\end{table}
...
Bij de modellering van de data is gekozen voor een klassiek sterschema, omdat dit type model goed aansluit bij rapportering en analytische workloads.

Alternatieven zoals een snowflake schema of een platte tabel werden bewust niet gekozen. Een snowflake model zou extra joins en complexiteit toevoegen voor rapportering, terwijl een platte tabel te veel redundantie en minder flexibele filtering zou veroorzaken. Het sterschema biedt in deze context de beste balans tussen eenvoud, performantie en uitbreidbaarheid voor BI toepassingen.

De centrale feitentabel \texttt{fact\_stats} bevat de wedstrijdstatistieken per speler per seizoen. Elke rij in deze tabel stelt de prestaties van één speler in één wedstrijd voor en bevat onder meer de volgende velden:
\begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{sterschema_scouting.png}
    \caption{Sterschema van het BNXT-scoutingdatawarehouse met \texttt{fact\_stats} en de dimensietabellen \texttt{dim\_player}, \texttt{dim\_team}, \texttt{dim\_games} en \texttt{dim\_season}.}
    \label{fig:sterschema}
\end{figure}
